makeObject <- function(outdata, outpriors, model, nsample=5000){
  ...
  if(is.null(outdata$modality.setting) || outdata$modality.setting==FALSE){
    if(is.null(outdata$covariates.setting) || outdata$covariates.setting==FALSE){
      ...
      suminfo = lapply(1:length(marginals.expected.accuracy.original), function(x) .summary.marginal(marginals.expected.accuracy.original[[x]],level=level))
      ...
      suminfo = lapply(1:length(marginals.expected.accuracy.transform), function(x) .summary.marginal(marginals.expected.accuracy.transform[[x]],level=level))
  ...
  if(est$misc$sample.flag){
    ...
    if(is.null(outdata$covariates.setting) || outdata$covariates.setting==FALSE){
      ...
      if(is.null(outdata$modality.setting) || outdata$modality.setting==FALSE){
        ...
        summary.RD.temp = .summary.samples(mean.RD.samples, level=level)
        summary.LDOR.temp = .summary.samples(mean.LDOR.samples, level=level)
        summary.LLRpos.temp = .summary.samples(mean.LLRpos.samples, level=level)
        summary.LLRneg.temp = .summary.samples(mean.LLRneg.samples, level=level)
        ...
        summary.LRpos.temp = lapply(1:mod.level, function(i) .summary.samples(mean.LRpos.samples[[i]], level=level))
        summary.LRneg.temp = lapply(1:mod.level, function(i) .summary.samples(mean.LRneg.samples[[i]], level=level))
        summary.DOR.temp = lapply(1:mod.level, function(i) .summary.samples(mean.DOR.samples[[i]], level=level))
        summary.RD.temp = lapply(1:mod.level, function(i) .summary.samples(mean.RD.samples[[i]], level=level))
        summary.LDOR.temp = lapply(1:mod.level, function(i) .summary.samples(mean.LDOR.samples[[i]], level=level))
        summary.LLRpos.temp = lapply(1:mod.level, function(i) .summary.samples(mean.LLRpos.samples[[i]], level=level))
        summary.LLRneg.temp = lapply(1:mod.level, function(i) .summary.samples(mean.LLRneg.samples[[i]], level=level))
    ...
    summary.fitted.RD.temp = t(apply(RD.samples, 1, function(x) .summary.samples(x, level=level)))
    rownames(summary.fitted.RD.temp) = studynames
    summary.fitted.LDOR.temp = t(apply(LDOR.samples, 1, function(x) .summary.samples(x, level=level)))
    rownames(summary.fitted.LDOR.temp) = studynames
    summary.fitted.LLRpos.temp = t(apply(LLRpos.samples, 1, function(x) .summary.samples(x, level=level)))
    rownames(summary.fitted.LLRpos.temp) = studynames
    summary.fitted.LLRneg.temp = t(apply(LLRneg.samples, 1, function(x) .summary.samples(x, level=level)))
