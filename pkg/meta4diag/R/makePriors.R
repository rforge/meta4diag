makePriors <- function(var.prior = "PC", var2.prior="PC", cor.prior = "PC",
  ...
  if(any(c(var.prior, var2.prior, cor.prior)=="invwishart")){
    ...
    halfCauchy = function(gamma){
      ...
      df_precision = data.frame(x = log(tau), y = log(pi_precision))
      df_distance = data.frame(x = sig, y = pi_variance*sig)
      return(list(variance=df_variance, precision=df_precision, distance=df_distance))
    }
    ...
    if(var.prior=="invgamma"){
      ...
      prior.table = paste(c("table:",cbind(priorlist$precision$x, priorlist$precision$y)), sep="", collapse=" ")
      ...
      prec1=list(prior = "logtnorm",param = c(var.par[1],1/var.par[2]), initial = prec1.init)
      ...
      tau.prior = INLA::inla.tmarginal(function(x) 1/x, var.par, n=1024)
      prior.table = paste(c("table:",cbind(log(tau.prior$x), log(tau.prior$y))), sep="", collapse=" ")
      prec1 = list(prior = prior.table, initial = prec1.init)
    ...
    if(var2.prior=="invgamma"){
      ...
      density$variance2 = .priorGammaV(a = var.par[1], b = var.par[2])
      density.distance$variance2 = .distGammaV(a = var.par[1], b = var.par[2])
    } else if(var2.prior=="pc"){
      prec2=list(prior = "pc.prec",param = var2.par, initial = prec2.init)
      density$variance2 = .priorPCV(u = var.par[1], alpha = var.par[2])
      density.distance$variance2 = .distPCV(u = var.par[1], alpha = var.par[2])
      ...
      prec2=list(prior = "logtnorm",param = c(var2.par[1],1/var2.par[2]), initial = prec2.init)
      ...
      tau.prior = INLA::inla.tmarginal(function(x) 1/x, var2.par, n=1024)
      prior.table = paste(c("table:",cbind(log(tau.prior$x), log(tau.prior$y))), sep="", collapse=" ")
      prec1 = list(prior = prior.table, initial = prec2.init)
    ...
    if(cor.prior=="normal"){
      ...
      prior.table = paste(c("table:",cbind(log(theta.prior$x), log(theta.prior$y))), sep="", collapse=" ")
